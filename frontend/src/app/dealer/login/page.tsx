...
export default function DealerLoginPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("dealerToken")) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res  = await fetch("http://localhost:5001/api/dealer/login", {
        method:  "POST",
      ...
      sessionStorage.setItem("dealer", JSON.stringify(data.dealer ?? data.user ?? {}))
      router.replace("/dealer/dashboard")
    } catch {
    ...
    <div style={{
      ...
      >
        <div style={{
          ...
          background: "rgba(16,185,129,0.12)", filter: "blur(80px)",
        }} />
        <div style={{
          position: "absolute", bottom: "-60px", right: "-60px",
          width: "280px", height: "280px", borderRadius: "50%",
          background: "rgba(5,150,105,0.1)", filter: "blur(60px)",
        }} />
        ...
        <div style={{ position: "relative" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "16px", lineHeight: 1.7, maxWidth: "340px" }}>
            Your dedicated portal for viewing assigned orders and managing your product catalogue.
          </p>

          <div style={{ marginTop: "48px", display: "flex", flexDirection: "column", gap: "16px" }}>
            {["View orders assigned to you", "Manage your product listings", "Track fulfilment status"].map(item => (
              <div key={item} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <span style={{ color: "rgba(255,255,255,0.55)", fontSize: "14px" }}>{item}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <input
                ...
                style={{
                  ...
                  padding: "0 16px", color: "white", fontSize: "15px",
                  outline: "none", width: "100%", boxSizing: "border-box",
                  transition: "border-color 0.2s, box-shadow 0.2s",
                }}
                onFocus={e => { e.target.style.borderColor = "rgba(16,185,129,0.7)"; e.target.style.boxShadow = "0 0 0 3px rgba(16,185,129,0.15)" }}
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  style={{
                    height: "48px", background: "rgba(255,255,255,0.06)",
                    border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px",
                    padding: "0 48px 0 16px", color: "white", fontSize: "15px",
                    outline: "none", width: "100%", boxSizing: "border-box",
                    transition: "border-color 0.2s, box-shadow 0.2s",
                  }}
                  onFocus={e => { e.target.style.borderColor = "rgba(16,185,129,0.7)"; e.target.style.boxShadow = "0 0 0 3px rgba(16,185,129,0.15)" }}
          ...
          <p style={{ textAlign: "center", marginTop: "28px", color: "rgba(255,255,255,0.2)", fontSize: "12px" }}>
            Restricted access · Authorised dealers only
          </p>
        </div>
      </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (min-width: 900px) { .dealer-left-panel { display: flex !important; } }
      `}</style>
    </div>
  )
}
