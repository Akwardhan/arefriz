...
import {
  MessageSquare, Package, User, Mail, Phone, Building2, Clock,
  CheckCircle2, Archive, Circle, Search, RefreshCw, Send,
  CornerDownRight, X, Loader2, ChevronDown,
} from "lucide-react"
...
const S: Record<InquiryStatus, {
  ...
  unread:   { label: "Unread",   color: "#2563EB", bg: "#EFF6FF", border: "#BFDBFE", glow: "rgba(37,99,235,0.12)",   icon: <Circle      className="h-3 w-3 fill-current" /> },
  open:     { label: "Open",     color: "#D97706", bg: "#FFFBEB", border: "#FDE68A", glow: "rgba(217,119,6,0.12)",   icon: <Clock       className="h-3.5 w-3.5" /> },
  closed:   { label: "Closed",   color: "#16A34A", bg: "#F0FDF4", border: "#BBF7D0", glow: "rgba(22,163,74,0.12)",   icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
  archived: { label: "Archived", color: "#94A3B8", bg: "#F8FAFC", border: "#E2E8F0", glow: "rgba(0,0,0,0.04)",       icon: <Archive     className="h-3.5 w-3.5" /> },
}

const resolveStatus = (s?: string): InquiryStatus =>
  s === "open" || s === "closed" || s === "archived" ? s : "unread"
...
export default function InquiriesPanel() {
  ...
  const TABS = [
    { key: "all" as const,      label: "All",      count: counts.all,      color: "#6366F1" },
    { key: "unread" as const,   label: "Unread",   count: counts.unread,   color: S.unread.color },
    { key: "open" as const,     label: "Open",     count: counts.open,     color: S.open.color },
    { key: "closed" as const,   label: "Closed",   count: counts.closed,   color: S.closed.color },
    { key: "archived" as const, label: "Archived", count: counts.archived, color: S.archived.color },
  ]
    ...
    <div className="min-h-screen" style={{ background: "#F8FAFC" }}>
      <div className="mx-auto max-w-6xl px-6 py-8 space-y-6">
          ...
          {TABS.map(({ key, label, count, color }) => {
            ...
            return (
              <button
                ...
                className="rounded-xl border bg-white p-3.5 text-left transition-all duration-150 hover:shadow-md active:scale-[0.98]"
                style={{
        ...
        {!loading && visible.length > 0 && (
          <div className="space-y-3">
            {visible.map(inq => (
              <InquiryCard key={inq._id} inq={inq} updateStatus={updateStatus} />
            ))}
          </div>
        )}
...
function InquiryCard({
  ...
  const TRANSITIONS = (
    [
      { key: "open" as InquiryStatus,     label: "Mark Open"   },
      { key: "closed" as InquiryStatus,   label: "Mark Closed" },
      { key: "archived" as InquiryStatus, label: "Archive"     },
    ]
  ...
  async function sendReply() {
    ...
    try {
      ...
      setTimeout(() => { setReplying(false); setReplyText(""); setReplyState("idle") }, 2000)
    } catch {
      setReplyState("error")
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: expanded ? `0 0 0 1px ${cfg.color}20, 0 4px 20px ${cfg.glow}` : "0 1px 3px rgba(0,0,0,0.06)",
      }}
    >
      <button type="button" onClick={() => setExpanded(v => !v)} className="w-full text-left transition-colors hover:bg-slate-50/60">
        <div className="flex items-center gap-4 px-5 py-4">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <span className="h-2.5 w-2.5 shrink-0 rounded-full"
              style={{ background: cfg.color, boxShadow: statusKey === "unread" ? `0 0 0 3px ${cfg.color}25` : "none" }} />
            <div className="min-w-0">
              <p className="text-sm font-semibold leading-none" style={{ color: "#0F172A" }}>{inq.name ?? "—"}</p>
              {inq.company && <p className="mt-0.5 text-xs" style={{ color: "#94A3B8" }}>{inq.company}</p>}
            </div>
          </div>

          {productName && (
            <div className="hidden shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 sm:flex"
              style={{ background: "#F1F5F9", border: "1px solid #E2E8F0" }}>
              <Package className="h-3 w-3 shrink-0" style={{ color: "#94A3B8" }} />
              <span className="max-w-[160px] truncate text-[0.72rem] font-medium" style={{ color: "#475569" }}>{productName}</span>
            </div>
          )}

          <span className="hidden shrink-0 items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold sm:inline-flex"
            style={{ background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}` }}>
            {cfg.icon}{cfg.label}
          </span>

          <p className="hidden shrink-0 text-xs md:block" style={{ color: "#94A3B8" }}>{fmtDate(inq.createdAt)}</p>
      ...
      {expanded && (
        <div className="border-t" style={{ borderColor: "#F1F5F9" }}>
          <div className="grid grid-cols-1 divide-y sm:grid-cols-3 sm:divide-x sm:divide-y-0" style={{ borderColor: "#F1F5F9" }}>

            <div className="px-5 py-5">
              <p className="mb-3 text-[0.65rem] font-semibold uppercase tracking-widest" style={{ color: "#CBD5E1" }}>Customer</p>
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={field} className="flex items-start gap-2.5">
                    ...
                    <span className="text-sm break-all" style={{ color: field === "name" ? "#1E293B" : "#475569", fontWeight: field === "name" ? 500 : 400 }}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="px-5 py-5">
              <p className="mb-3 text-[0.65rem] font-semibold uppercase tracking-widest" style={{ color: "#CBD5E1" }}>Regarding</p>
              {productName ? (
                <div className="flex items-start gap-3 rounded-xl p-3.5" style={{ background: "#F8FAFC", border: "1px solid #E2E8F0" }}>
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg" style={{ background: "#EFF6FF" }}>
                    <Package className="h-4 w-4" style={{ color: "#2563EB" }} strokeWidth={1.75} />
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold leading-snug" style={{ color: "#1E293B" }}>{productName}</p>
                    {productId && <p className="mt-1 font-mono text-[0.68rem]" style={{ color: "#94A3B8" }}>SKU · {productId.toUpperCase().slice(0, 10)}</p>}
                  </div>
                </div>
              ) : (
            ...
            <div className="px-5 py-5">
              <p className="mb-3 text-[0.65rem] font-semibold uppercase tracking-widest" style={{ color: "#CBD5E1" }}>Message</p>
              {inq.message ? (
                <div className="rounded-xl p-3.5" style={{ background: "#F8FAFC", border: "1px solid #E2E8F0" }}>
                  <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: "#334155" }}>{inq.message}</p>
                </div>
              ) : (
              ...
              {replyState === "sent" ? (
                ...
                <>
                  <textarea rows={4} value={replyText} onChange={e => setReplyText(e.target.value)}
                    ...
                    onBlur={e  => { e.currentTarget.style.borderColor = "#E2E8F0"; e.currentTarget.style.boxShadow = "none" }}
                  />
                  {replyState === "error" && <p className="mt-1.5 text-xs" style={{ color: "#EF4444" }}>Failed to send — please try again.</p>}
                  <div className="mt-2.5 flex items-center justify-end gap-2">
                    <button onClick={() => { setReplying(false); setReplyText("") }}
                      className="rounded-lg px-4 py-2 text-xs font-semibold transition-colors hover:bg-slate-100" style={{ color: "#64748B" }}>
                      Cancel
                    </button>
                    <button onClick={sendReply} disabled={replyState === "sending" || !replyText.trim()}
                      ...
                      {replyState === "sending" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t px-5 py-3"
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {TRANSITIONS.map(t => {
                const tcfg = S[t.key]
                return (
                  <button key={t.key} onClick={e => { e.stopPropagation(); updateStatus(inq._id, t.key) }}
            ...
            <button onClick={e => { e.stopPropagation(); setReplying(v => !v) }}
